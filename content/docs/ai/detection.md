...
<script>
  const t1t = 5;
  document.getElementById('t1i').setAttribute('src', `https://${root}/${id}/thumbnails/thumbnail.jpg?time=${t1t}s&height=720`);

  const t1cData = await fetch(`/api/ai/detections?id=${id}&t=${t1t}`);
